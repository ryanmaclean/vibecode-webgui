...
func getCommand(name string) commands.Command {
	...
	default:
		if cmd := commands.GetScriptProxyCommand(name); cmd != nil {
			return cmd
		}
		return nil
	}
...
func printHelp() {
	...
	fmt.Println()
	fmt.Println("Script Parity (Proxy) Commands:")
	fmt.Println("  query-app-security     Query Application Security events")
	fmt.Println("  query-cloud-security   Query Cloud Security events")
	fmt.Println("  query-error-tracking   Query Error Tracking issues")
	fmt.Println("  query-data-streams     Query Data Streams Monitoring")
	fmt.Println("  query-hosts            Query infrastructure hosts")
	fmt.Println("  query-session-replay   Query Session Replay data")
	fmt.Println("  query-profiling        Query profiling data")
	fmt.Println("  query-ci-tests         Query CI test results")
	fmt.Println("  manage-logs-pipelines  Manage log pipelines")
	fmt.Println("  manage-custom-metrics  Manage custom metrics")
	fmt.Println("  manage-restriction-policies  Manage restriction policies")
	fmt.Println("  manage-webhooks        Manage webhooks")
	fmt.Println("  verify-setup           Verify Datadog setup and configuration")
	fmt.Println("  investigate-service    Comprehensive service investigation")
	fmt.Println("  example-monitored-script  Example monitored script with Datadog instrumentation")
	fmt.Println("  test-monitoring        Run monitoring test harness")
	fmt.Println()
