...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    cat << 'EOF'
Capacity Scale (Go CLI proxy)

Usage:
  capacity-scale.sh [options]
...
EOF
    resolve_go_cli "$SCRIPT_DIR" >/dev/null 2>&1 && "$(resolve_go_cli "$SCRIPT_DIR")" capacity-scale --help || true
    exit 0
fi

run_go_cli "$SCRIPT_DIR" "capacity-scale" "$@"
