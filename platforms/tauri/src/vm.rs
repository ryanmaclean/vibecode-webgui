...
pub async fn vm_list(app: AppHandle) -> Result<Vec<VMInfo>, String> {
    ...
    // Look for .img files
    let entries =
        std::fs::read_dir(&vm_dir).map_err(|e| format!("Failed to read VM directory: {}", e))?;
...
pub async fn vm_stop(vm_name: String) -> Result<String, String> {
    ...
    if output.status.success() {
        ...
        Err(format!(
            "Failed to stop VM '{}': VM may not be running",
            vm_name
        ))
    }
}
...
pub async fn vm_setup_first_run(app: AppHandle) -> Result<String, String> {
    ...
    Ok(format!(
        "Setup complete: {} VM files copied to {}",
        copied_count,
        user_vm_dir.display()
    ))
}
...
pub async fn vm_start_openvscode(app: AppHandle) -> Result<String, String> {
    ...
    if !forwarded {
        println!(
            "⚠️  Could not set up automatic port forwarding. VM may need manual SSH configuration."
        );
    }
