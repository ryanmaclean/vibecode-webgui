...
- `<name>.img` - Disk image
- `<name>-seed.iso` - Cloud-init seed ISO for configuration
- `<name>-efi.nvram` - NVRAM/EFI variables for UEFI boot
...
~/vibecode-webgui/dist/vm-images/
├── vibecode-ide.img
├── vibecode-ide-seed.iso
├── vibecode-ide-efi.nvram
├── vibecode-nodejs.img
├── vibecode-nodejs-seed.iso
├── vibecode-nodejs-efi.nvram
... (and so on)
```

3. VibeCode will automatically discover all VMs with matching `.img`, `-seed.iso`, and `-efi.nvram` files.
...
- Observability framework imports (Datadog, StatsD)
- VM discovery implementation (all 6 VMs)
- Multi-VM management support
- VM lifecycle management
- Metrics collection capability
- OpenTelemetry support
- Performance monitoring
- UI components (status display, control buttons)
...
**Current Test Results**: 100% pass rate (23/23 tests passing) ✅
- ✅ All build, code signing, and functionality tests pass
- ✅ Application launches successfully
- ✅ All observability features verified
...
```bash
# Should show .img, -seed.iso, -efi.nvram for each VM
cd ~/vibecode-webgui/dist/vm-images
ls -lh *.img *-seed.iso *-efi.nvram
```
...
- **VM Discovery**: Pattern matching on `.img` + `-seed.iso` + `-efi.nvram`
...
cd azure/SwiftUI-Apps/VibeCodeSwift
...
./build-vibecode-multivm.sh

# The app will be created at:
# azure/SwiftUI-Apps/VibeCode.app
```
...
- [ ] **Swift 5.9 or later** - Included with Xcode or Command Line Tools
- [ ] **Command Line Tools** - Install via `xcode-select --install`
- [ ] **VM images** - Located in `~/vibecode-webgui/dist/vm-images/`
  - Each VM requires: `.img`, `-seed.iso`, `-efi.nvram` files
  - At least one complete VM set for testing
...
```bash
codesign --force --deep --sign - VibeCode.app
```

See [BUILD.md](BUILD.md) for detailed signing instructions, troubleshooting, and packaging for distribution.
...
- `azure/SwiftUI-Apps/build-vibecode-multivm.sh` - Build script
