...
```bash
~/.vfkit/vms/vibecode-alpine/launch.sh &
```
...
Or run the helper:
```bash
chmod +x scripts/vfkit/BUILD_ALL_NOW.sh
./scripts/vfkit/BUILD_ALL_NOW.sh
```
...
| `scripts/vfkit/BUILD_ALL_NOW.sh` | Helper script (shows instructions) |
| `scripts/vfkit/build-services-on-host.sh` | Documentation/reference |
| `scripts/vfkit/build-tiny-*.sh` | Individual service build scripts |
| `scripts/vfkit/build-all-tiny-services.sh` | Auto-exec orchestration (tried, needs work) |
