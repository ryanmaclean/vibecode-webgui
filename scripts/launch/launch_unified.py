...
Launches the Unified Services VM with SSH, OpenVSCode, Valkey, and PostgreSQL.
"""
...
UNIFIED_INITRAMFS = AZURE_DIR / "unified-services-optimized.cpio.gz"
...
def kill_running_vms() -> None:
    ...
    run_cmd(["killall", "UnifiedServicesVibeCode"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"{Colors.BLUE}  Unified Services VM Quick Launch{Colors.NC}")
    print(f"{Colors.BLUE}================================={Colors.NC}")
    print()
    ...
    if not UNIFIED_INITRAMFS.exists():
    ...
    shutil.copy2(UNIFIED_INITRAMFS, NODEJS_INITRAMFS)
