...
echo -e "${BLUE}=================================${NC}"
echo -e "${BLUE}  PostgreSQL VM Quick Launch${NC}"
echo -e "${BLUE}=================================${NC}"
echo ""

# Kill any running VMs
echo "Stopping any running VMs..."
killall PostgreSQLVibeCode 2>/dev/null || true
...
if [ ! -f ~/vibecode-webgui/azure/postgresql-standalone.cpio.gz ]; then
    echo "ERROR: PostgreSQL initramfs not found!"
    exit 1
fi
...
cp postgresql-standalone.cpio.gz nodejs-complete.cpio.gz
...
if [ -z "$CONSOLE_LOG" ]; then
    ...
    if [ -n "$VM_IP" ]; then
        ...
        echo "  psql -h $VM_IP -U postgres -d vibecode"
        echo ""
        echo "Test Connection:"
        echo "  pg_isready -h $VM_IP -p 5432"
        echo ""
    else
