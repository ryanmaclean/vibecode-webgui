...
echo -e "${BLUE}=================================${NC}"
echo -e "${BLUE}  Unified Services VM Quick Launch${NC}"
echo -e "${BLUE}=================================${NC}"
echo ""

# Kill any running VMs
echo "Stopping any running VMs..."
killall UnifiedServicesVibeCode 2>/dev/null || true
...
if [ ! -f ~/vibecode-webgui/azure/unified-services-optimized.cpio.gz ]; then
    echo "ERROR: Unified Services initramfs not found!"
    exit 1
fi

# Launch VM
echo "Launching Unified Services VM..."
...
cp unified-services-optimized.cpio.gz nodejs-complete.cpio.gz
