...
# Deploy Datadog Container Monitoring Dashboard
# Uses Datadog API to create or update the container metrics dashboard
...
log() {
    ...
    dd_info "$1" "script:deploy-datadog-container-dashboard"
}

warn() {
    echo -e "${YELLOW}[$(date +'%Y-%m-%d %H:%M:%S')] WARNING: $1${NC}"
    dd_warn "$1" "script:deploy-datadog-container-dashboard"
}

error() {
    echo -e "${RED}[$(date +'%Y-%m-%d %H:%M:%S')] ERROR: $1${NC}"
    dd_error "$1" "script:deploy-datadog-container-dashboard"
    exit 1
}
...
DASHBOARD_FILE="$PROJECT_ROOT/infrastructure/datadog/dashboards/container-metrics-dashboard.json"
BACKUP_DIR="$PROJECT_ROOT/infrastructure/datadog/dashboards/.backups"
...
check_existing_dashboard() {
    ...
    # Search for existing "Container Resource Monitoring" dashboard
    local existing_id=$(echo "$body" | jq -r '.dashboards[] | select(.title == "Container Resource Monitoring") | .id' | head -n1)
...
backup_dashboard() {
    ...
    local backup_file="$BACKUP_DIR/container-metrics-dashboard-${dashboard_id}-$(date +%Y%m%d-%H%M%S).json"
...
create_dashboard() {
    ...
    if [ "$http_code" = "200" ]; then
        ...
        dd_metric "datadog.dashboard.created" "1" "count" "dashboard:container-monitoring"
        echo "$dashboard_url"
        return 0
    else
...
update_dashboard() {
    ...
    if [ "$http_code" = "200" ]; then
        ...
        dd_metric "datadog.dashboard.updated" "1" "count" "dashboard:container-monitoring"
        echo "$dashboard_url"
        return 0
    else
...
rollback_dashboard() {
    ...
    if [ "$http_code" = "200" ]; then
        ...
        dd_metric "datadog.dashboard.rollback" "1" "count" "dashboard:container-monitoring"
        return 0
    else
...
main() {
    log "Starting Datadog Container Monitoring Dashboard Deployment"
    echo
...
usage() {
    ...
    echo
    echo "Deploy or update Datadog Container Resource Monitoring dashboard"
    echo
    ...
    echo
    echo "  # Dry run"
    echo "  DD_API_KEY=xxx DD_APP_KEY=yyy $0 --dry-run"
    echo
    echo "  # Rollback to previous version"
    echo "  DD_API_KEY=xxx DD_APP_KEY=yyy $0 --rollback infrastructure/datadog/dashboards/.backups/container-metrics-dashboard-xyz-20260214-120000.json"
    echo
}
...
if [ -n "${ROLLBACK_FILE:-}" ]; then
    ...
    if [ -z "$DASHBOARD_ID" ]; then
        DASHBOARD_ID=$(basename "$ROLLBACK_FILE" | sed -E 's/container-metrics-dashboard-([^-]+)-.*/\1/')
        if [ -z "$DASHBOARD_ID" ]; then
