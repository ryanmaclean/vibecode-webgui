...
def find_initramfs(
    ...
    if custom_initramfs.exists():
        print(f"+ Using custom rootfs: {custom_initramfs} ({get_file_size(custom_initramfs)})")
...
def launch_vm(
    ...
    """
    Launch the Alpine VM with vfkit.
    ...
    initramfs_custom = rootfs_dir / "alpine-vibecode-rootfs.cpio.gz"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Launch Alpine Linux ARM64 VM with vfkit"
    )
    parser.add_argument(
        "--cpus",
        type=int,
