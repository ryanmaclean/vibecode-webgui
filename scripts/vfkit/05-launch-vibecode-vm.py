...
_dd_service = "vfkit-05-launch-vibecode-vm"
...
def get_vm_config() -> dict[str, str | int]:
    ...
    return {
        ...
        "memory": int(os.environ.get("VFKIT_MEMORY", "4096")),
...
def verify_project_dir(paths: dict[str, Path], config: dict[str, str | int]) -> None:
...
def build_vfkit_command(
    ...
    """Build the vfkit command line with virtio-fs."""
...
def print_config(config: dict[str, str | int], project_dir: Path) -> None:
    ...
    print()
    print("Kernel cmdline: console=hvc0 root=/dev/vda rw quiet")
    print()
...
def main() -> int:
    ...
    try:
        ...
        verify_project_dir(paths, config)
        console_log = prepare_console_log(paths)
        print()

        print_config(config, paths["project_dir"])
