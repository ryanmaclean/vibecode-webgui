...
VIBECODE_SHARE_TAG="vibecode"
VIBECODE_MOUNT_POINT="/mnt/vibecode"
...
echo "📁 VibeCode project directory: $PROJECT_DIR"
if [[ ! -d "$PROJECT_DIR" ]]; then
    echo "❌ Project directory not found: $PROJECT_DIR"
    exit 1
fi
echo "   Share tag: $VIBECODE_SHARE_TAG"
echo "   Mount point in VM: $VIBECODE_MOUNT_POINT"
echo ""
...
echo "Shared Directory:  ${PROJECT_DIR} -> ${VIBECODE_MOUNT_POINT}"
echo ""

# Kernel command line
# Note: We don't add virtiofs mount here, we'll do it in the init script
...
# Add directory sharing (virtio-fs)
VFKIT_CMD+=(
    --device
    "virtio-fs,sharedDir=$PROJECT_DIR,mountTag=$VIBECODE_SHARE_TAG"
)
...
echo "  - VibeCode WebGUI: http://localhost:3000"
