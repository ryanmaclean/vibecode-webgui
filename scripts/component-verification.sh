...
check "Datadog Secret Created" "kubectl get secret datadog-secret -n '$TEST_NAMESPACE' || kubectl get secret datadog-secrets -n '$TEST_NAMESPACE'"
check "PostgreSQL Secret Created" "kubectl get secret postgres-credentials -n '$TEST_NAMESPACE'"
check "API Key in Secret" "(kubectl get secret datadog-secret -n '$TEST_NAMESPACE' -o jsonpath='{.data.api-key}' 2>/dev/null || kubectl get secret datadog-secrets -n '$TEST_NAMESPACE' -o jsonpath='{.data.api-key}' 2>/dev/null) | (base64 -d 2>/dev/null || base64 -D) | grep -q 'test-key'"
...
check "API Key Secret Reference" "grep -q 'apiKeyExistingSecret.*datadog-secret' '$PROJECT_ROOT/helm/vibecode-platform/values-dev.yaml' || grep -q 'apiKeyExistingSecret.*datadog-secrets' '$PROJECT_ROOT/helm/vibecode-platform/values-dev.yaml'"
