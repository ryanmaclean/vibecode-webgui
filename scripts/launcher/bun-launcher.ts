...
class BunLauncher {
  ...
  /**
   * Initialize the launcher environment
   */
  ...
  /**
   * Verify VM assets (kernel, initramfs) are available
   */
  ...
  /**
   * Download Alpine Linux kernel for VM
   */
  ...
  /**
   * Create minimal initramfs for OpenVSCode Server
   */
  private async createInitramfs(): Promise<void> {
    ...
    if (existsSync(initramfsScript)) {
      ...
      // Fallback: try to use existing vibecode-alpine assets
  ...
  /**
   * Start the VM with OpenVSCode Server
   */
  async startVM(): Promise<void> {
    log('info', 'Starting OpenVSCode VM...');

    // Build the Swift VM launcher
  ...
  private async buildVMLauncher(buildDir: string): Promise<void> {
    ...
    // Use existing valkey VM launcher as base
  ...
  async startServer(): Promise<void> {
    ...
    // Check for OpenVSCode Server binary
    const serverPaths = [
  ...
  /**
   * Start OpenVSCode Server via Docker
   */
  ...
  private setupHotReload(): void {
    ...
    // Bun's file watcher for hot reload
    // Note: In a full implementation, we'd use Bun.FileSystemWatcher
  ...
  private async setupPortForwarding(): Promise<void> {
    ...
    // Port forwarding is handled by the VM configuration
