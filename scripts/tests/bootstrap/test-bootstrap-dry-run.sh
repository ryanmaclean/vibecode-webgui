...
EOF
  ...
  sed 's/validate_aks_cluster$/validate_aks_cluster_dry_run/g' "$SOURCE_SCRIPT" >> "$dry_run_script"
...
EOF
  sed -i "s#\./scripts/aks-datadog-setup.sh#${DATADOG_SCRIPT}#g" "$dry_run_script"
  sed -i "s#\./scripts/aks-postgresql-setup.sh#${POSTGRES_SCRIPT}#g" "$dry_run_script"
  sed -i "s#\./scripts/aks-app-deploy.sh#${APP_SCRIPT}#g" "$dry_run_script"
...
for script in "${required_scripts[@]}"; do
  if [ -x "$script" ]; then
    echo "   ✅ ${script#"${BOOTSTRAP_TEST_REPO_ROOT}"/}"
  else
    echo "   ❌ ${script#"${BOOTSTRAP_TEST_REPO_ROOT}"/} missing or not executable"
    exit 1
  fi
done
...
for script in "${required_scripts[@]}"; do
  if bash -n "$script"; then
    echo "   ✅ ${script#"${BOOTSTRAP_TEST_REPO_ROOT}"/} syntax OK"
  else
    echo "   ❌ ${script#"${BOOTSTRAP_TEST_REPO_ROOT}"/} syntax error"
    exit 1
  fi
done
