...
# Copy the Swift source (without shebang)
...
echo "Building ValkeyVM..."
cd "$BUILD_DIR"
swift build -c release 2>&1 | grep -v "warning:" || true

if [ ! -f ".build/release/ValkeyVM" ]; then
    echo "✗ Build failed"
    exit 1
fi
...
codesign --entitlements "${BUILD_DIR}/entitlements.plist" -f -s - ".build/release/ValkeyVM"
