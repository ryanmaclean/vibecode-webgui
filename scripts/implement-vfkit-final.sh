...
# VibeCode vfkit Final Working Implementation
# Create a functional macOS VM using vfkit with proper bootloader

set -e

echo "🍎 VibeCode vfkit Final Working Implementation"
echo "==============================================="
...
# Create a working vfkit VM with proper bootloader
echo "🚀 Creating working vfkit VM..."

# Try macOS VM with proper bootloader syntax
echo "🍎 Testing macOS VM with proper bootloader..."

# Start vfkit with macOS bootloader
vfkit \
    --cpus "$VM_CPUS" \
    --memory "$VM_MEMORY" \
    --bootloader "macos" \
