...
exports.DeploymentWebviewProvider = void 0;
const vscode = __importStar(require("vscode"));
const axios_1 = __importDefault(require("axios"));
class DeploymentWebviewProvider {
    constructor(_extensionUri, baseUrl = 'http://localhost:3000') {
    ...
    async deployProject(providerId) {
        try {
            ...
            await vscode.window.withProgress({
                ...
                const response = await axios_1.default.post(`${this.baseUrl}/api/deployment/deploy`, {
                    provider: providerId,
                ...
                while (attempts < maxAttempts) {
                    ...
                    try {
                        const statusResponse = await axios_1.default.get(`${this.baseUrl}/api/deployment/status/${deploymentId}`);
    ...
    async refreshDeployments() {
        try {
            const response = await axios_1.default.get(`${this.baseUrl}/api/deployment/list`);
    ...
    async setupProvider(providerId) {
        const panel = vscode.window.createWebviewPanel('providerSetup', `Setup ${providerId}`, vscode.ViewColumn.One, { enableScripts: true });
        panel.webview.html = this._getProviderSetupHtml(providerId);
        panel.webview.onDidReceiveMessage(async (message) => {
            if (message.type === 'saveConfig') {
                try {
                    await axios_1.default.post(`${this.baseUrl}/api/deployment/configure`, {
                        provider: providerId,
    ...
    async viewDeploymentLogs(deploymentId) {
        try {
            const response = await axios_1.default.get(`${this.baseUrl}/api/deployment/logs/${deploymentId}`);
    ...
    _getProviderSetupHtml(providerId) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
...
exports.DeploymentWebviewProvider = DeploymentWebviewProvider;
DeploymentWebviewProvider.viewType = 'vibeCodeDeployment';
//# sourceMappingURL=deployment-provider.js.map