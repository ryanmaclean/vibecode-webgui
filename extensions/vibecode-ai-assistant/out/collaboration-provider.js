...
class CollaborationProvider {
    ...
    async startCollaborativeSession(sessionName, inviteEmails) {
        try {
            if (!sessionName.trim()) {
            ...
            const response = await axios_1.default.post(`${this._apiBaseUrl}/api/collaboration/start`, {
                name: sessionName,
            ...
            if (this._currentSession) {
                ...
                vscode.window.showInformationMessage(`Collaborative session "${sessionName}" started! Session ID: ${sessionId}`, 'Copy Link').then(selection => {
    ...
    async joinCollaborativeSession(sessionId, accessCode) {
        try {
            if (!sessionId.trim()) {
            ...
            const response = await axios_1.default.post(`${this._apiBaseUrl}/api/collaboration/join`, {
    ...
    async endCollaborativeSession() {
        try {
            if (!this._currentSession) {
                vscode.window.showWarningMessage('No active session to end');
                return;
            }
            const confirmation = await vscode.window.showWarningMessage(`Are you sure you want to end the session "${this._currentSession.name}"?`, 'Yes', 'Cancel');
            if (confirmation !== 'Yes') {
                return;
            }
            await axios_1.default.post(`${this._apiBaseUrl}/api/collaboration/end`, {
    ...
    async shareFile(filePath) {
        try {
            ...
            await axios_1.default.post(`${this._apiBaseUrl}/api/collaboration/share-file`, {
                session_id: this._currentSession.id,
                file_path: filePath
            });
    ...
    async unshareFile(filePath) {
        try {
            if (!this._currentSession) {
                vscode.window.showWarningMessage('No active collaboration session');
                return;
            }
            await axios_1.default.post(`${this._apiBaseUrl}/api/collaboration/unshare-file`, {
                session_id: this._currentSession.id,
                file_path: filePath
            });
    ...
    async sendChatMessage(message) {
        try {
            ...
            await axios_1.default.post(`${this._apiBaseUrl}/api/collaboration/chat`, {
                session_id: this._currentSession.id,
                message: message
            });
        }
    ...
    async updateCursorPosition(file, line, character) {
        try {
            if (!this._currentSession) {
                return;
            }
            await axios_1.default.post(`${this._apiBaseUrl}/api/collaboration/cursor`, {
                session_id: this._currentSession.id,
                file: file,
                line: line,
                character: character
    ...
    async _loadActiveSessions() {
        try {
            const response = await axios_1.default.get(`${this._apiBaseUrl}/api/collaboration/sessions`);
    ...
    _startSessionMonitoring() {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                const response = await axios_1.default.get(`${this._apiBaseUrl}/api/collaboration/session/${sessionId}`);
...
CollaborationProvider.viewType = 'vibeCodeCollaboration';
