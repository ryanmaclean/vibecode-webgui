...
exports.MonitoringProvider = void 0;
...
class MonitoringProvider {
    ...
    async _fetchSystemMetrics() {
        const response = await axios_1.default.get(`${this._apiBaseUrl}/api/monitoring/system`);
        return response.data;
    }
    async _fetchUsageMetrics() {
        const response = await axios_1.default.get(`${this._apiBaseUrl}/api/monitoring/usage`);
        return response.data;
    }
    async _fetchPerformanceMetrics() {
        const response = await axios_1.default.get(`${this._apiBaseUrl}/api/monitoring/performance`);
        return response.data;
    }
    async _fetchAlertMetrics() {
        const response = await axios_1.default.get(`${this._apiBaseUrl}/api/monitoring/alerts`);
        return response.data;
    }
    async _fetchDeploymentMetrics() {
        const response = await axios_1.default.get(`${this._apiBaseUrl}/api/monitoring/deployments`);
        return response.data;
    }
    _getMockSystemMetrics() {
        return {
            cpu_usage: 45.2,
            memory_usage: 68.9,
            disk_usage: 23.1,
    ...
    async _exportMetrics(format) {
        try {
            const response = await axios_1.default.get(`${this._apiBaseUrl}/api/monitoring/export`, {
                params: { format }
            });
    ...
    async _acknowledgeAlert(alertId) {
        try {
            await axios_1.default.post(`${this._apiBaseUrl}/api/monitoring/alerts/${alertId}/acknowledge`);
            await this._refreshMetrics();
    ...
    async _clearAllAlerts() {
        const confirmation = await vscode.window.showWarningMessage('Are you sure you want to clear all alerts?', 'Yes', 'Cancel');
        if (confirmation !== 'Yes') {
            return;
        }
        try {
            await axios_1.default.delete(`${this._apiBaseUrl}/api/monitoring/alerts`);
            await this._refreshMetrics();
...
MonitoringProvider.viewType = 'vibeCodeMonitoring';
