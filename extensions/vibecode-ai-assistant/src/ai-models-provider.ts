...
interface OrchestrationConfig {
    load_balancing: 'round_robin' | 'least_cost' | 'performance_based' | 'manual';
...
export class AIModelsTreeItem extends vscode.TreeItem {
    constructor(
        ...
        if (model) {
            ...
            this.iconPath = new vscode.ThemeIcon(
                model.available ? 'check-all' : 'error',
                model.available ? undefined : new vscode.ThemeColor('errorForeground')
            );
        }
...
export class AIModelsProvider implements vscode.TreeDataProvider<AIModelsTreeItem> {
    private _onDidChangeTreeData: vscode.EventEmitter<AIModelsTreeItem | undefined | null | void> = new vscode.EventEmitter<AIModelsTreeItem | undefined | null | void>();
    readonly onDidChangeTreeData: vscode.Event<AIModelsTreeItem | undefined | null | void> = this._onDidChangeTreeData.event;

    private _apiBaseUrl = 'http://localhost:3000';
    private _models: AIModel[] = [];
    private _usage: { [modelId: string]: ModelUsage } = {};
    private _config: OrchestrationConfig | null = null;
    ...
    getTreeItem(element: AIModelsTreeItem): vscode.TreeItem {
        return element;
    }

    getChildren(element?: AIModelsTreeItem): Thenable<AIModelsTreeItem[]> {
        if (!element) {
    ...
    private _getRootItems(): AIModelsTreeItem[] {
        return [
    ...
    private _getModelItems(): AIModelsTreeItem[] {
        return this._models.map(model => {
            const usage = this._usage[model.id];
            return new AIModelsTreeItem(
                model.name,
    ...
    private _getUsageItems(): AIModelsTreeItem[] {
        const usageItems: AIModelsTreeItem[] = [];
        ...
        for (const usage of sortedUsage.slice(0, 10)) { // Top 10
            ...
            if (model) {
                const item = new AIModelsTreeItem(
                    model.name,
                    vscode.TreeItemCollapsibleState.None,
                    undefined,
                    usage,
    ...
    private _getOrchestrationItems(): AIModelsTreeItem[] {
        const items: AIModelsTreeItem[] = [];
        
        if (this._config) {
            items.push(new AIModelsTreeItem(
                `Load Balancing: ${this._config.load_balancing.replace('_', ' ')}`,
                vscode.TreeItemCollapsibleState.None,
                undefined,
                undefined,
                'config-item'
            ...
            items.push(new AIModelsTreeItem(
                `Failover: ${this._config.failover_enabled ? 'Enabled' : 'Disabled'}`,
                vscode.TreeItemCollapsibleState.None,
                undefined,
                undefined,
                'config-item'
            ...
            items.push(new AIModelsTreeItem(
                `Max Retries: ${this._config.max_retries}`,
                vscode.TreeItemCollapsibleState.None,
                undefined,
                undefined,
                'config-item'
            ...
            items.push(new AIModelsTreeItem(
                `Timeout: ${this._config.timeout_ms}ms`,
                vscode.TreeItemCollapsibleState.None,
                undefined,
                undefined,
                'config-item'
    ...
    private async _loadModels(): Promise<void> {
        try {
            const response = await axios.get(`${this._apiBaseUrl}/api/ai/models`);
    ...
    private async _loadUsage(): Promise<void> {
        try {
            const response = await axios.get(`${this._apiBaseUrl}/api/ai/usage`);
    ...
    private async _loadOrchestrationConfig(): Promise<void> {
        try {
            const response = await axios.get(`${this._apiBaseUrl}/api/ai/orchestration/config`);
    ...
    private _getFallbackModels(): AIModel[] {
        return [
            {
    ...
    private _getDefaultOrchestrationConfig(): OrchestrationConfig {
        return {
    ...
    // Command handlers
    public async testModel(model: AIModel): Promise<void> {
        try {
            const response = await vscode.window.withProgress({
                ...
                const testResponse = await axios.post(`${this._apiBaseUrl}/api/ai/test`, {
                    model_id: model.id,
    ...
    public async configureModel(model: AIModel): Promise<void> {
    ...
    public async showUsageDetails(usage: ModelUsage): Promise<void> {
        const model = this._models.find(m => m.id === usage.model_id);
        if (!model) {
            return;
        }
...
• Tokens per Request: ${Math.round(usage.total_tokens / usage.requests_count)}
    ...
    public async configureOrchestration(): Promise<void> {
        await this._showAdvancedConfiguration();
    }

    private async _updateOrchestrationConfig(property: keyof OrchestrationConfig, value: any, action: 'add' | 'remove' | 'set'): Promise<void> {
        try {
            const payload: any = { property, value, action };
            await axios.put(`${this._apiBaseUrl}/api/ai/orchestration/config`, payload);
    ...
    private async _showAdvancedConfiguration(): Promise<void> {
        const panel = vscode.window.createWebviewPanel(
            'orchestrationConfig',
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                case 'updateConfig':
                    try {
                        await axios.put(`${this._apiBaseUrl}/api/ai/orchestration/config`, {
    ...
    private _getConfigurationWebviewContent(): string {
