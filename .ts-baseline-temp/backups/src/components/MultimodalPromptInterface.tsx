...
export default function MultimodalPromptInterface({ 
  ...
  const [_currentAudio, setCurrentAudio] = useState<string | null>(null);
  
  // Voice and multimodal state
  const [isListening, setIsListening] = useState(false);
  const [isRecording, setIsRecording] = useState(false);
  const [voiceEnabled, _setVoiceEnabled] = useState(true);
  const [autoPlayVoice, _setAutoPlayVoice] = useState(false);
  
  // Sample and demo state
  const [_selectedSample, setSelectedSample] = useState<SampleScenario | null>(null);
