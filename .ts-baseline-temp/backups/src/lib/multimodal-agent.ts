...
Be encouraging, technically accurate, and provide working code examples.`;
   ...
   */
  private async generateImprovementSuggestions(files: ProjectFile[], _response: string): Promise<ImprovementSuggestion[]> {
   ...
   */
  private inferFilePath(codeBlock: CodeBlock, _input: MultimodalInput): string {
...
class VoiceProcessor {
  ...
  async transcribe(_audio: Blob | File): Promise<string> {
  ...
  async generateSpeech(_text: string, _voiceSettings: Record<string, unknown>): Promise<string> {
