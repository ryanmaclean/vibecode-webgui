 ...
 *
 * Database connection pooling for efficient resource management and high performance.
 * Implements automatic scaling, connection validation, idle cleanup, and comprehensive
 * metrics tracking for production database workloads.
 *
 * @performance Connection pooling reduces database connection overhead from ~50ms
 * per connection to near-zero for subsequent requests using pooled connections.
 */
...
/**
 * Configuration interface for connection pool behavior
 */
export interface ConnectionPoolConfig {
  ...
  /** Interval in milliseconds for connection health validation */
  validationIntervalMs: number;

  /** Whether to log pool errors to console */
  logErrors: boolean;
}

/**
 * Internal connection wrapper with metadata for tracking
 */
interface PoolConnection {
  ...
  /** Timestamp of connection creation (milliseconds since epoch) */
  createdAt: number;
}

/**
 * Database Connection Pool for Prisma clients
 *
 * Manages a pool of Prisma connections with automatic scaling, health validation,
 * and idle connection cleanup. Provides efficient resource utilization for
 * high-throughput database workloads.
 *
 * Features:
 * - Automatic connection scaling between min and max
 * - Periodic health validation with reconnection
 * - Idle connection cleanup to free resources
 * - Comprehensive metrics tracking
 * - Thread-safe connection acquisition and release
 *
 * @performance Maintains minimum connections for instant availability while
 * cleaning up idle connections to conserve resources.
 *
 * @example
...
export class ConnectionPool {
   ...
   *
   * Initializes the pool with minimum connections and starts periodic
   * validation timer for connection health checks and idle cleanup.
   *
   * @param config - Connection pool configuration options
   * @param config.min - Minimum connections (default: 2)
   * @param config.max - Maximum connections (default: 10)
   * @param config.acquireTimeoutMs - Acquisition timeout (default: 5000ms)
   * @param config.idleTimeoutMs - Idle timeout (default: 30000ms)
   * @param config.validationIntervalMs - Validation interval (default: 60000ms)
   * @param config.logErrors - Enable error logging (default: true)
   *
   * @example
