...
import  from '@/lib/templates/versioning'
...
export class TemplateMarketplace {
  ...
  private _featuredTemplates: string[] = []
  ...
  async addReview(templateId: string, review: Omit<TemplateReview, 'id' | 'createdAt' | 'helpful'>): Promise<{
    ...
    try {
      // Validate review
      const _validatedReview = templateReviewSchema.parse({
   ...
   */
  async forkTemplate(marketplaceId: string, _userId: string): Promise<{
    success: boolean
    forkId?: string
    error?: string
  }> {
