...
class AppDelegate: NSObject, NSApplicationDelegate, VZVirtualMachineDelegate {
    ...
    // VM paths
    private let vmBundlePath = NSHomeDirectory() + "/VibeCode VMs/UbuntuGUI VM.bundle/"
    ...
    private func createMainDiskImage() {
        ...
        do {
            // 64GB disk (sparse on APFS - starts small, grows as needed)
            try diskFileHandle.truncate(atOffset: 64 * 1024 * 1024 * 1024)
            print("✅ Created 64GB disk (sparse): \(mainDiskImagePath)")
        } catch {
    ...
    func createVirtualMachine() {
        ...
        if needsInstall {
            ...
            disks.append(createUSBMassStorageDeviceConfiguration())
        } else {
        ...
        disks.append(createBlockDeviceConfiguration())
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        window.title = "UbuntuGUI VibeCode - GUI Linux VM"
        window.center()
        ...
        if !FileManager.default.fileExists(atPath: vmBundlePath) {
            ...
            openPanel.title = "Select UbuntuGUI ISO Image"
            openPanel.message = "Choose an ARM64 Linux ISO (Ubuntu 26.x recommended)"
            openPanel.canChooseFiles = true
            openPanel.allowsMultipleSelection = false
            openPanel.allowedContentTypes = [.init(filenameExtension: "iso")!]
            
            openPanel.begin { result in
                if result == .OK {
                    self.installerISOPath = openPanel.url!
                    print("📦 Installing from: \(self.installerISOPath!.path)")
                    self.configureAndStartVirtualMachine()
                } else {
                    print("❌ ISO not selected")
                    NSApp.terminate(nil)
                }
            }
        } else {
            needsInstall = false
            print("🚀 Booting existing VM from ASIF disk")
