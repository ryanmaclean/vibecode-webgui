...
- Add a new workflow (see issue #405) that:
  1. Dispatches `deploy-docs.yml` with `docs_system=nextjs`.
  2. Downloads the artifact and publishes the container image to the chosen registry.
  3. Invokes the platform-specific deploy command (Azure `az webapp`, AWS `aws apprunner update-service`).
  4. Runs the smoke test (`curl -fsS https://docs.vibecode.dev/api/readyz`).
- Store deployment credentials in GitHub environments (`docs-next-staging`, `docs-next-prod`).
