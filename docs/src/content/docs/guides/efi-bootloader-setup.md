...
1. **`scripts/vfkit/validate-grub-installation.py`**: Validates GRUB installation
2. **`scripts/vfkit/install-grub-alpine.py`**: Installs GRUB on Alpine disk images
3. **`scripts/vfkit/manage-efi-boot-entries.py`**: Manages EFI boot entries
4. **`scripts/vfkit/test-grub-boot.py`**: End-to-end test workflow
...
python scripts/vfkit/test-grub-boot.py --vm-name=test-vm --disk-size=5
```
...
```bash
python scripts/vfkit/validate-grub-installation.py <disk_path>
```
...
3. Trigger boot discovery: `python scripts/vfkit/manage-efi-boot-entries.py discover`
