...
### September 19, 2025
- Licensing hygiene complete: removed all `@upstash/*` and `@vercel/kv` usage and references across the repository. Standardized on Valkey/Redis-only clients to maintain MIT/BSD/Apache-compatible stack.
- Docs pipeline restored: reintroduced flagship pages with explicit slugs and fixed HTML assertions; docs test script now passes end-to-end.
