...
if [ ! -x "scripts/aks-bootstrap.sh" ]; then
  echo "❌ scripts/aks-bootstrap.sh is not executable"
  exit 1
fi
...
bash -n scripts/aks-bootstrap.sh && echo "   ✓ Script syntax is valid"
...
temp_script=$(mktemp)
sed '/^main() {/,$d' scripts/aks-bootstrap.sh > "$temp_script"
cat >> "$temp_script" << 'EOF'
