...
export class AppleContainerWorkspaceService {
  ...
  async createWorkspace(request: WorkspaceRequest): Promise<WorkspaceResponse> {
    console.info(`📦 Creating Apple Container workspace for: ${request.projectName}`)
    ...
    console.info(`📁 Created workspace directory: ${workspacePath}`)

    // 5. Write project files to workspace
    await this.writeProjectFiles(workspacePath, request.files)
    console.info(`📝 Wrote ${Object.keys(request.files).length} files`)
    ...
    console.info(`🚀 Starting code-server container...`)
    ...
    console.info(`✅ Container started: ${result.id}`)

    // 8. Wait for code-server to be ready
    await this.waitForCodeServer(port)
    console.info(`🌐 Code-server ready at http://localhost:${port}`)
  ...
  async deleteWorkspace(workspaceId: string): Promise<void> {
    ...
    if (container) {
      ...
      console.info(`✅ Container removed: ${container.id}`)
    }
  }
...
const result = await appleContainer.list()
  ...
  private async installDependencies(containerId: string): Promise<void> {
    ...
    console.info(`⚠️  Dependencies will be installed on first access`)
  }
