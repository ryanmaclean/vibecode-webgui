...
```bash
python scripts/initramfs-builder/validate-grub-installation.py <disk_path> [efi_store_path] [--json]
```
...
```bash
python scripts/initramfs-builder/install-grub-alpine.py <disk_path> [vm_name] [--method=auto|vm|chroot]
```
...
```bash
python scripts/initramfs-builder/manage-efi-boot-entries.py create <vm_name> <disk_path> <efi_store_path>
python scripts/initramfs-builder/manage-efi-boot-entries.py discover <disk_path> <efi_store_path>
python scripts/initramfs-builder/manage-efi-boot-entries.py list <efi_store_path>
```
...
```bash
python scripts/initramfs-builder/test-grub-boot.py [--vm-name=test-vm] [--disk-size=5] [--cleanup]
```
...
python scripts/initramfs-builder/test-grub-boot.py

# Custom VM name and disk size
python scripts/initramfs-builder/test-grub-boot.py --vm-name=my-test-vm --disk-size=10

# Clean up after test
python scripts/initramfs-builder/test-grub-boot.py --cleanup
```
...
# Run with tracing
ddtrace-run python scripts/initramfs-builder/validate-grub-installation.py <disk_path>
```
...
- [README-PYTHON-SCRIPTS.md](../../../scripts/initramfs-builder/README-PYTHON-SCRIPTS.md) - Detailed script documentation
