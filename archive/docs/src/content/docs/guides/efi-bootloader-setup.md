---
title: "EFI Bootloader Setup Guide"
description: "Complete guide to setting up GRUB EFI bootloader for Alpine Linux VMs"
sidebar:
  order: 1
---
...
**Python Scripts (Recommended - with ddtrace support):**
1. **`scripts/initramfs-builder/validate-grub-installation.py`**: Validates GRUB installation
2. **`scripts/initramfs-builder/install-grub-alpine.py`**: Installs GRUB on Alpine disk images
3. **`scripts/initramfs-builder/manage-efi-boot-entries.py`**: Manages EFI boot entries
4. **`scripts/initramfs-builder/test-grub-boot.py`**: End-to-end test workflow

**Bash Scripts (Legacy - for compatibility):**
1. **`scripts/initramfs-builder/install-grub-alpine.sh`**: Installs GRUB on Alpine disk images
2. **`scripts/initramfs-builder/prepare-vm-with-grub.sh`**: Complete VM preparation workflow
3. **`scripts/initramfs-builder/manage-efi-boot-entries.sh`**: Manages EFI boot entries

**Swift Utilities:**
1. **`VibeCodeSwift/Sources/EFIBootManager/EFIBootManager.swift`**: Swift utility for EFI boot management

### Quick Start

#### Option 1: Validate GRUB Installation (Python - Recommended)

```bash
# Validate GRUB installation
python scripts/initramfs-builder/validate-grub-installation.py \
    ~/.vibecode/vms/vibecode-postgresql/disk/root.img \
    ~/.vibecode/vms/vibecode-postgresql/efi/efi.nvram
```

#### Option 2: Install GRUB (Python - Recommended)

```bash
# Install GRUB on existing disk image
python scripts/initramfs-builder/install-grub-alpine.py \
    ~/.vibecode/vms/vibecode-postgresql/disk/root.img \
    postgresql
```

#### Option 3: Manage EFI Boot Entries (Python - Recommended)

```bash
# Create EFI boot entry
python scripts/initramfs-builder/manage-efi-boot-entries.py create postgresql \
    ~/.vibecode/vms/vibecode-postgresql/disk/root.img \
    ~/.vibecode/vms/vibecode-postgresql/efi/efi.nvram

# Discover boot entries (boot VM once)
python scripts/initramfs-builder/manage-efi-boot-entries.py discover \
    ~/.vibecode/vms/vibecode-postgresql/disk/root.img \
    ~/.vibecode/vms/vibecode-postgresql/efi/efi.nvram
```

#### Option 4: End-to-End Test (Python)

```bash
# Run complete test workflow
python scripts/initramfs-builder/test-grub-boot.py --vm-name=test-vm --disk-size=5
```

**Note:** Python scripts provide ddtrace integration for observability. See [GRUB EFI Python Scripts](./grub-efi-python-scripts.md) for details.
...
qemu-img create -f raw ~/.vibecode/vms/vibecode-postgresql/disk/root.img 10G
```
...
```bash
vfkit \
    --bootloader efi,variable-store=~/.vibecode/vms/vibecode-postgresql/efi/efi.nvram,create \
    --device virtio-blk,path=~/.vibecode/vms/vibecode-postgresql/disk/root.img \
...
The `install-grub-alpine.py` script:
...
```bash
python scripts/initramfs-builder/validate-grub-installation.py <disk_path>
```
...
3. Trigger boot discovery: `python scripts/initramfs-builder/manage-efi-boot-entries.py discover`

### EFI Variable Store Issues

If EFI variable store is corrupted:
1. Delete the existing EFI store file
2. Create a new one: Use `EFIBootManager.createEFIVariableStore()`
...
1. **Always use the validation script** before booting VMs
...
- [GRUB EFI Python Scripts](./grub-efi-python-scripts.md) - Detailed Python script documentation
