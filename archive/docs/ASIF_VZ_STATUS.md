...
### Build Scripts
- **`scripts/initramfs-builder/02-download-alpine-kernel.sh`** - Alpine kernel downloader
- **`scripts/initramfs-builder/03-create-alpine-rootfs.sh`** - Rootfs builder
- **`scripts/initramfs-builder/04-launch-alpine-vm.sh`** - VM launcher
...
1. Download Alpine ARM64 kernel (~20MB)
   ```bash
   ./scripts/initramfs-builder/02-download-alpine-kernel.sh
   ```

2. Create minimal initramfs (~3MB)
   ```bash
   ./scripts/initramfs-builder/03-create-alpine-rootfs.sh
   ```
...
```bash
cd ~/.vibecode/vms/vibecode-alpine
...
6. **Examples exist** - Check scripts/vz/ and scripts/initramfs-builder/
7. **Performance is measured** - See test results above

**Next agent should:**
- Run `./scripts/initramfs-builder/02-download-alpine-kernel.sh`
