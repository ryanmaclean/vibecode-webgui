...
vfkit --cpus 2 --memory 1024 \
  ...
  --initrd ~/.vibecode/vms/vibecode-valkey/rootfs/auto-exec.cpio.gz \
  --kernel-cmdline "console=hvc0 root=/dev/vda rw quiet" \
  --device virtio-blk,path=~/.vibecode/vms/vibecode-valkey/disk/root.img \
  --device virtio-net,nat,mac=52:54:00:12:34:57 \
  --device virtio-serial,logFilePath=~/.vibecode/vms/vibecode-valkey/logs/console.log \
  --device virtio-rng
```

**PID:** 26844
**Location:** `~/.vibecode/vms/vibecode-valkey/`
...
4. **~/.vibecode/vms/valkey-vz/** (directory structure)
   - Disk images
   - Logs
   - Configuration
...
# VM infrastructure
~/.vibecode/vms/valkey-vz/
~/.vibecode/vms/vibecode-valkey/  # vfkit-based
~/.vibecode/vms/vibecode-alpine/kernel/  # Shared kernel
...
tail -f ~/.vibecode/vms/vibecode-valkey/logs/console.log
