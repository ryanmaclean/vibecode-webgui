...
import { VectorDbErrorType } from './vector-db-error-handler-new';
...
export const DB_ERROR_PATTERNS: Record<string, Record<string, DbErrorPattern>> = {
  ...
  postgres: {
    ...
    connection: {
      ...
      types: [VectorDbErrorType.CONNECTION]
    },
  ...
  'cognitive-search': {
    ...
    vector: {
      ...
      types: [VectorDbErrorType.VECTOR_OPERATION_FAILED]
    },
...
export function categorizeErrorWithProvider(error: any, provider: string): VectorDbErrorType {
  ...
  for (const [category, pattern] of Object.entries(providerPatterns)) {
    // Check status codes
