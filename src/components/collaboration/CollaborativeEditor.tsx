...
export const CollaborativeEditor: React.FC<CollaborativeEditorProps> = ({
  ...
  const initializeCollaboration = useCallback(async () => {
    try {
      ...
      const extensions: Extension[] = [
        ...
        yCollab(yText, (session.provider?.awareness as Awareness | undefined) ?? null),
        EditorView.theme({
          '&': {
            height: '100%',
            fontSize: '14px'
      ...
      if (session.provider && typeof session.provider.on === 'function') {
      ...
      if (session.provider?.awareness && typeof session.provider.awareness.on === 'function') {
  ...
  useEffect(() => {
    ...
    const awareness = sessionRef.current.provider.awareness as Awareness
