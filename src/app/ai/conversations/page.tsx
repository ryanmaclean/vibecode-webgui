...
export default function AIConversationsPage() {
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      ...
      const parsed: Conversation[] = (data.conversations ?? []).map(
    ...
    <div className="space-y-6">
      ...
      {error && !isLoading && (
        <div className="bg-white rounded-lg border border-red-200 p-12 text-center">
          <MessageSquare className="mx-auto h-12 w-12 text-red-300 mb-4" />
      ...
      {/* Main content - hidden during loading or error */}
      {!isLoading && !error && (
        <>
          {/* Stats Row */}
