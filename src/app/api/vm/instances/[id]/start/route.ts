/**
 * VM Instance Start API
 *
 * POST /api/vm/instances/[id]/start - Start a VM instance
 */
...
/**
 * POST /api/vm/instances/[id]/start
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    log.info('Starting VM instance', {
      userId: session.user?.id,
      vmId: id
    });

    // Get pool manager and start VM
    ...
    const result = await poolManager.startVM(id);

    if (!result.success) {
      log.warn('Failed to start VM', {
        userId: session.user?.id,
        vmId: id,
        error: result.error
      });
    ...
    log.error('Failed to start VM instance', {
