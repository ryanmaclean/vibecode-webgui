...
async function handlePOST(request: AuthenticatedRequest): Promise<NextResponse> {
  ...
  console.counter('vibecode.api.ai.chat.requests', 1, {
  ...
  try {
    ...
    console.counter('vibecode.ai.chat.completions', 1, {
      model,
      stream: stream.toString(),
      user_role: request.user?.role || 'unknown'
    });

    console.gauge('vibecode.ai.chat.response_length', response.length, {
      model,
      endpoint: '/api/ai/chat'
    });
    ...
    console.counter('vibecode.api.ai.chat.errors', 1, {
    ...
    if (error instanceof Error && error.message.includes('validation')) {
      console.security('ai_validation_failure', {
        endpoint: '/api/ai/chat',
        error: error.message,
        userId: request.user?.id,
        requestId
