...
export class ChatWebviewProvider implements vscode.WebviewViewProvider {
    ...
    public async focusChat(): Promise<void> {
        if (this._view) {
            this._view.show?.(true);
        }
    }

    private async handleChatMessage(text: string, includeContext: boolean): Promise<void> {
        if (!text.trim()) {
            return;
        }
    ...
    private async handleNonStreamingResponse(messages: ChatMessage[]): Promise<void> {
    ...
    private getCurrentCodeContext(): string {
    ...
    private updateChatUI(message: ChatMessage, isStreaming: boolean = false): void {
        if (this._view) {
