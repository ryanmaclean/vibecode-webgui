...
export class EnhancedVectorStore {
  ...
  async deleteDocuments(options: {
    workspaceId?: number;
    fileIds?: number[];
  }): Promise<{ totalDeleted: number; providerResults: Record<string, number> }> {
    const providerName = this.selectOptimalProvider('delete', {}, {});
    const providerResults: Record<string, number> = {
      [providerName]: 0
    };

    if (this.config.enableMetrics) {
      this.recordOperation(providerName, 'delete', Date.now(), true);
    }

    return {
      totalDeleted: providerResults[providerName],
      providerResults
    };
  }
  ...
  async healthCheck(): Promise<{
    ...
    return {
      ...
      providers: Array.from(this.providers.keys()).map((provider) => ({
        id: provider,
        available: this.providers.get(provider)?.isHealthy ?? false,
