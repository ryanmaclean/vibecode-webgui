...
export class CachedAzureEmbeddingService extends AzureEmbeddingService {
  ...
  constructor(
    ...
    if (enableCache) {
      console.info('🚀 Cached Azure Embedding Service initialized with caching enabled')
    }
  }
  ...
  public async generateEmbedding(text: string, options: CachedEmbeddingOptions = {}): Promise<number[]> {
    ...
    try {
      ...
      if (result.cached) {
        console.info(`📦 Embedding cache hit - saved API call for "${text.substring(0, 30)}..."`)
      } else {
        console.info(`🔥 Embedding generated and cached for "${text.substring(0, 30)}..."`)
      }
      ...
      console.info('🔄 Falling back to direct embedding generation')
      return super.generateEmbedding(text, options)
    }
  }
...
// Process texts in parallel for better performance
    ...
    console.info(`📊 Batch embedding generation: ${cachedCount}/${totalCount} from cache (${Math.round((cachedCount/totalCount) * 100)}% hit rate)`)
  ...
  public async preloadEmbeddings(
    ...
    console.info(`🔥 Preloading ${commonTexts.length} common embeddings into cache...`)
    ...
    console.info(`✅ Preload complete: ${newlyGenerated} new embeddings generated and cached`)
  }
  ...
  public async clearEmbeddingCache(): Promise<number> {
    console.info('🗑️ Clearing embedding cache for Azure service...')
  ...
  public setCacheEnabled(enabled: boolean): void {
    ...
    console.info(`Cache ${enabled ? 'enabled' : 'disabled'} for Azure Embedding Service`)
  }
