...
export class ConnectionPool<T> {
  ...
  private async initializeMinConnections(): Promise<void> {
    try {
      ...
      console.info(`Initializing connection pool with ${initialConnections} connections`);
      ...
      console.info(`Connection pool initialized with ${this.pool.length} connections`, {
  ...
  private async pruneConnections(): Promise<void> {
    ...
    if (prunedCount > 0) {
      console.debug(`Pruned ${prunedCount} connections from pool`, {
  ...
  private async createNewConnection(): Promise<PoolConnection<T>> {
    ...
    while (attempts < this.config.maxConnectionAttempts) {
      try {
        ...
        console.debug('Created new connection in pool', { 
  ...
  private async validateAndAcquire(pooledConnection: PoolConnection<T>): Promise<T> {
    ...
    try {
      ...
      // Connection is valid
      console.debug('Acquired connection from pool', { 
        connectionId: pooledConnection.id,
  ...
  public async release(connection: T): Promise<void> {
    ...
    console.debug('Released connection back to pool', { 
      connectionId: pooledConnection.id,
      activeConnections: this.activeConnections,
      poolSize: this.pool.length
    });
    ...
    if (this.waiting.length > 0) {
      ...
      if (waiter) {
        try {
          ...
          console.debug('Provided released connection to waiting request', { 
            connectionId: pooledConnection.id,
            waitTime
          });
        } catch (error) {
  ...
  public async close(): Promise<void> {
    ...
    console.info(`Connection pool closed, ${closedCount} connections terminated`);
