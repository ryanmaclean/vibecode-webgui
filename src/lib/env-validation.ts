...
// Helper for decimal string fields with defaults
const decimalStringWithDefault = (defaultValue: string) =>
  z.preprocess(
    (val) => val ?? defaultValue,
    z.string().regex(/^\d*\.?\d+$/).transform(Number)
  )

// Helper for boolean string fields with defaults
const booleanStringWithDefault = (defaultValue: string) =>
  z.preprocess(
    (val) => val ?? defaultValue,
...
const envSchema = z.object({
  ...
  // Rate limiting
  RATE_LIMIT_MAX_REQUESTS: numericStringWithDefault('100'),
  RATE_LIMIT_WINDOW_MS: numericStringWithDefault('900000'),
  ...
  // Feature flags
  ENABLE_MONITORING: booleanStringWithDefault('true'),
  ENABLE_PERFORMANCE_TRACKING: booleanStringWithDefault('true'),
  ENABLE_ERROR_TRACKING: booleanStringWithDefault('true'),
  ENABLE_AI_ANALYTICS: booleanStringWithDefault('true'),

  // Health check configuration
  HEALTH_CHECK_TIMEOUT: numericStringWithDefault('5000'),
  HEALTH_CHECK_INTERVAL: numericStringWithDefault('30000'),

  // AI configuration
  DEFAULT_LLM_MODEL: z.string().default('gpt-4-turbo'),
  MAX_TOKENS: numericStringWithDefault('4000'),
  TEMPERATURE: decimalStringWithDefault('0.7'),

  // Storage and sizing
  AI_REQUEST_TIMEOUT: numericStringWithDefault('30000'),
  AI_MAX_TOKENS: numericStringWithDefault('4096'),
})
