...
describe('WebSocketConnectionPool', () => {
  ...
  describe('Message Handling', () => {
    ...
    it('should send messages', async () => {
      ...
      await expect(
        connectionPool.sendMessage(mockConnection.id, message)).resolves.not.toThrow()})
    ...
    it('should handle send errors', async () => {
      ...
      await expect(
        connectionPool.sendMessage(mockConnection.id, 'test')).rejects.toThrow('Send failed')})
      ...
      global.WebSocket = jest.fn().mockImplementation(() => {
        setTimeout(() => {
          mockWebSocket.on.mock.calls
            ...
            .forEach(call => call[1](new Error('Connection failed')))}, 10);
        return mockWebSocket
      }) as any

      try {
        await connectionPool.getConnection(mockWebSocketUrl)} catch (error) {
        // Expected to fail
      }
