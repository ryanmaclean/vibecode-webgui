...
// y-indexeddb not available in this project, skipping mock
...
describe('CollaborationManager', () => {
  ...
  describe('Multi-User Scenarios', () => {
    test('should handle multiple users in same session', async () => {
      ...
      expect(activeUsers.find((u: any) => u.id === mockUser.id)).toBeDefined()
      expect(activeUsers.find((u: any) => u.id === secondUser.id)).toBeDefined()
    })
  ...
  describe('Error Handling', () => {
    test('should handle WebSocket connection errors gracefully', async () => {
      ...
      const onCallback = mockProvider.on.mock.calls.find((call: any) => call[0] === 'connection-error')
