...
test.describe('Health Check Tests', () => {
  ...
  test('should have working database test endpoint', async ({ page }) => {
    ...
    // Verify database response structure
    expect(dbData).toHaveProperty('status');
    expect(dbData).toHaveProperty('message');
    expect(dbData).toHaveProperty('result');
    expect(dbData).toHaveProperty('timestamp');
    
    // Verify database connection is working
    expect(dbData.status).toBe('success');
    expect(dbData.message).toContain('Database connection working');
    
    // Verify we got actual data back
    expect(dbData.result).toBeDefined();
    expect(Array.isArray(dbData.result)).toBeTruthy();
    expect(dbData.result[0]).toHaveProperty('test');
    expect(dbData.result[0].test).toBe(1);
    
    console.log('Database test successful:', dbData.message);
  });
