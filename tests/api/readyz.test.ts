/**
 * Kubernetes Readiness Probe API Tests
...
describe('/api/readyz - Kubernetes Readiness Probe', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('GET /api/readyz', () => {
    it('should return ready status with 200', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/readyz');
    ...
    it('should handle validation failure gracefully', async () => {
      ...
      const { GET } = await import('@/app/api/readyz/route');
      const request = new NextRequest('http://localhost:3000/api/readyz?invalid=param');
    ...
    it('should respond within acceptable time for k8s probing', async () => {
      const { GET } = await import('@/app/api/readyz/route');
      const request = new NextRequest('http://localhost:3000/api/readyz');
      ...
      expect(response.status).toBe(200);
      // Readiness probe should respond within 200ms in test environment
  ...
  describe('Difference from Liveness Probe', () => {
    it('should return "ready" status (not "healthy")', async () => {
      const { GET } = await import('@/app/api/readyz/route');
      const request = new NextRequest('http://localhost:3000/api/readyz');
      ...
      // Readiness should return 'ready', liveness returns 'healthy'
      expect(data.status).toBe('ready');
