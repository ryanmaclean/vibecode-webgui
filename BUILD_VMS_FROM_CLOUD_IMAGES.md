...
```bash
mkdir -p ~/.vfkit/cloud-init/valkey

# Create user-data
cat > ~/.vfkit/cloud-init/valkey/user-data <<EOF
#cloud-config
...
# Create meta-data
cat > ~/.vfkit/cloud-init/valkey/meta-data <<EOF
instance-id: vibecode-valkey-001
local-hostname: vibecode-valkey
EOF

# Generate cloud-init ISO
hdiutil makehybrid -o ~/.vfkit/cloud-init/valkey/seed.iso \
  -hfs -joliet -iso -default-volume-name cidata \
  ~/.vfkit/cloud-init/valkey/
```
...
cp ~/.vfkit/images/alpine-3.22-base.raw ~/.vfkit/vms/vibecode-valkey/disk.raw

# Launch with vfkit
vfkit \
  --cpus 2 \
  --memory 1024 \
  --bootloader efi,variable-store=~/.vfkit/vms/vibecode-valkey/efi.bin,create \
  --device virtio-blk,path=~/.vfkit/vms/vibecode-valkey/disk.raw \
  --device virtio-blk,path=~/.vfkit/cloud-init/valkey/seed.iso \
  --device virtio-net,nat,mac=52:54:00:12:34:59 \
  --device virtio-serial,logFilePath=~/.vfkit/vms/vibecode-valkey/console.log \
  --device virtio-rng
```
